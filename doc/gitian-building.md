...
*Setup instructions for a gitian build of Agenor Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Agenor
...
to the Agenor GitHub release page.
...
- [Building Agenor Core](#building-agenor-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/agenor-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/agenor-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/agenor-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/agenor-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/agenor-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/agenor-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/AgenorCoin/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@agenor.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/agenor-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/agenor-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/agenor-osx-*-build.assert
```
